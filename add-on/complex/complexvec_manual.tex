...
int main() {
    ...
    Complex2d a( 1, 2, 3,  4); // (1, 2), (3, 4)
    Complex2d b(-2, 1, 0, -3); // (-2, 1), (0, -3)
...
\section{A note on infinity} \label{Infinity}
There is no single universally agreed-upon way to represent infinity in complex numbers. This library uses the component-wise representation common in C++. The real and imaginary parts of a complex number may individually be finite or infinite. 
\vspacesmall

A tricky case occurs when multiplying by infinity:

\begin{example}
\label{exampleInfinity}
\end{example} % frame disappears if I put this after end lstlisting
\begin{lstlisting}[frame=single]
// Example for infinity
double inf = std::numeric_limits<double>::infinity(); // inf
Complex2d a(inf, 1.);             // (inf, 1.)
Complex2d x1 = a * 2.;            // (inf, 1.) * 2      = (inf, 2.)
Complex2d x2 = a * Complex2d(2.); // (inf, 1.) * (2, 0) = (inf, NaN)
\end{lstlisting}
\vspacesmall

While the real number 2 and the complex number (2, 0) are equivalent, they may behave differently when multiplying by infinity. The variable x1 in the above example has multiplied both components of a by 2. The variable x2 does the same, but also multiplies both parts by zero, which produces inf * 0 = NaN.
\vspacesmall

While this library allows representations for pure real numbers, there is no representation for pure imaginary numbers. Therefore, there is no analogous dilemma when multiplying a complex number containing infinity by a pure imaginary number.
\vspacesmall
